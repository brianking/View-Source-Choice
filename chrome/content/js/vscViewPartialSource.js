...
#   Brian King <http://briks.si/kontakt/>
#
...
function onLoadViewPartialSource(docUrl, docCharset, reference, context)
{
  // check the view_source.wrap_long_lines pref and set the menuitem's checked attribute accordingly
  if (vscprefs) {
    try {
      var wraplonglinesPrefValue = vscprefs.getBoolPref('view_source.wrap_long_lines');
    ...
    try {
      //document.getElementById("menu_highlightSyntax").setAttribute("checked", vscprefs.getBoolPref("view_source.syntax_highlight"));
    } catch (e) {
    }
  } else {
  ...
  if (context == 'selection')
    viewPartialSourceForSelection(reference);
  else
    viewPartialSourceForFragment(reference, context);
...
function viewPartialSourceForSelection(selection)
{
  ...
  var vsWhere = vscprefs.getIntPref("extensions.vsc.open");
  var selUrl = "view-source:data:text/html;charset=utf-8," + encodeURIComponent(tmpNode.innerHTML);
  vscClass.loadSource(selUrl, vsWhere, "sel");
}
...
function drawSelection()
{
  getBrowser().contentDocument.title =
    vscClass.getViewSourceBundle().getString("viewSelectionSourceTitle");
...
function viewPartialSourceForFragment(node, context)
{
  ...
  // serialize
  var title = vscClass.getViewSourceBundle().getString("viewMathMLSourceTitle");
